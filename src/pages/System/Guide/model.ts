...
const Model: ModelType = {
  ...
  effects: {
    * submitRegularForm({ payload }, { call, select }) {
      const id = yield select((state: any) => state.systemAndGuide.fieldValue.id)
      try {
        payload = {
          ...payload,
          id,
          guidePower: payload.guidePower ? 1 : 0,
        }
    ...
    * getConfig({ payload }, { call, put }) {
      try {
        ...
        yield put({
          ...
          payload: {
            ...res.data,
            guidePower: res.data.guidePower ? true : false
          }
        })
      } catch (error) {
        console.log(error)
