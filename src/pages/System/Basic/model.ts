...
const Model: ModelType = {
  ...
  effects: {
    * submitRegularForm({ payload }, { call, select }) {
      const id = yield select((state: any) => state.systemAndBasic.fieldValue.id)
      try {
        payload = {
          ...payload,
          id,
          service: payload.service ? 1 : 0,
          family: payload.family ? 1 : 0
        }
    ...
    * getConfig({ payload }, { call, put }) {
      try {
        ...
        yield put({
          ...
          payload: {
            ...res.data,
            service: res.data.service ? true : false,
            family: res.data.family ? true : false
          }
        })
      } catch (error) {
        console.log(error)
